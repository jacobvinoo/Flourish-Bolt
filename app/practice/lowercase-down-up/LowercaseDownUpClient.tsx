...
// This array defines the practice steps for the lowercase down-up letters workbook.
  ...
  return (
    <div className="space-y-3">
      <div className={`border-2 border-dashed rounded-xl p-8 text-center transition-all duration-300 cursor-pointer ${dragActive ? isKidsMode ? 'border-purple-400 bg-gradient-to-br from-purple-100 to-pink-100 scale-105' : 'border-blue-400 bg-blue-50' : isKidsMode ? 'border-purple-300 hover:border-purple-400 hover:bg-gradient-to-br hover:from-purple-50 hover:to-pink-50 hover:scale-102' : 'border-gray-300 hover:border-gray-400 hover:bg-gray-50'} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        onClick={() => { if (!disabled && !uploading) document.getElementById('file-upload-down-up')?.click(); }}>
        ...
        <Input id="file-upload-down-up" type="file" accept="image/jpeg,image/png,image/jpg" onChange={handleChange} className="hidden" disabled={disabled || uploading} />
      </div>
      {error && (
...
export default function LowercaseDownUpClient({ user, profile, initialSubmissions }: PracticePageClientProps) {
  ...
  const simulateAIAnalysis = (file: File) => {
    ...
    return {
      ...
      accuracy: accuracy,
      feedbackTip: 'Great work on your down-up letters! Focus on keeping the bumps consistent in height.',
  ...
  const handleGradingComplete = async () => {
    ...
    try {
      ...
      setCompletedSteps(prev => new Set(prev).add(downUpWorkbookSteps[currentStep].id));
      
      setTimeout(() => {
        setUploadSuccess(false);
        setSelectedFile(null);
        if (currentStep < downUpWorkbookSteps.length - 1) {
  ...
  const goToNextStep = () => { if (currentStep < downUpWorkbookSteps.length - 1) setCurrentStep(currentStep + 1); };

  const currentWorksheet = downUpWorkbookSteps[currentStep];
  const progressPercentage = (completedSteps.size / downUpWorkbookSteps.length) * 100;
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {isKidsMode && (
          <div className="border-0 shadow-xl mb-8 bg-gradient-to-r from-indigo-500 to-blue-500 text-white overflow-hidden rounded-2xl">
            <div className="pt-6 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  <div className="text-2xl font-bold">{completedSteps.size} / {downUpWorkbookSteps.length}</div>
            ...
            <div className={`border-0 shadow-xl overflow-hidden rounded-2xl ${isKidsMode ? `bg-gradient-to-br ${currentWorksheet.color} text-white` : 'bg-white border'}`}>
              ...
              <div className="p-6 pt-0 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold">{currentStep + 1} of {downUpWorkbookSteps.length}</span>
                  <Button onClick={goToNextStep} disabled={currentStep === downUpWorkbookSteps.length - 1} variant="outline">Next<ChevronRight className="h-5 w-5 ml-2" /></Button>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="p-6 border-0 shadow-xl rounded-2xl bg-white/50">
              ...
              <div className="space-y-2 max-h-[500px] overflow-y-auto pr-2">
                {downUpWorkbookSteps.map((step, index) => (
                  <button
                    key={step.id}
