// app/practice/uppercase-full-curves/UpperspaceFullCurvesClient.tsx
'use client';
...
// This array defines the practice steps for the uppercase full curves workbook.
const fullCurvesWorkbookSteps: WorksheetStep[] = [
  {
    id: 'letter-c',
    title: 'Worksheet 2.3.1: Letter C',
    friendlyTitle: 'The letter C!',
    description: 'Practice the open curve of the letter "C".',
    kidsDescription: "Let's draw a big curve like a smile to make a \"C\"!",
    level: 2,
  ...
  {
    id: 'letter-g',
    title: 'Worksheet 2.3.2: Letter G',
    friendlyTitle: 'The letter G!',
    description: 'Master the curve and small line of the letter "G".',
  ...
  {
    id: 'letter-o',
    title: 'Worksheet 2.3.3: Letter O',
  ...
  {
    id: 'letter-q',
    title: 'Worksheet 2.3.4: Letter Q',
    friendlyTitle: 'The letter Q!',
    description: 'Master the circle and small diagonal line of the letter "Q".',
  ...
  {
    id: 'letter-s',
    title: 'Worksheet 2.3.5: Letter S',
  ...
  return (
    <div className="space-y-3">
      <div className={`border-2 border-dashed rounded-xl p-8 text-center transition-all duration-300 cursor-pointer ${dragActive ? isKidsMode ? 'border-purple-400 bg-gradient-to-br from-purple-100 to-pink-100 scale-105' : 'border-blue-400 bg-blue-50' : isKidsMode ? 'border-purple-300 hover:border-purple-400 hover:bg-gradient-to-br hover:from-purple-50 hover:to-pink-50 hover:scale-102' : 'border-gray-300 hover:border-gray-400 hover:bg-gray-50'} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        onClick={() => { if (!disabled && !uploading) document.getElementById('file-upload-full-curves')?.click(); }}>
        ...
        <Input id="file-upload-full-curves" type="file" accept="image/jpeg,image/png,image/jpg" onChange={handleChange} className="hidden" disabled={disabled || uploading} />
      </div>
      {error && (
...
export default function UpperspaceFullCurvesClient({ user, profile }: PracticePageClientProps) {
  ...
  const simulateAIAnalysis = (file: File) => {
    ...
    return {
      ...
      feedbackTip: 'Great work on your curved letters! Focus on making smooth, consistent curves.',
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const currentWorksheet = fullCurvesWorkbookSteps[currentStep];
  ...
  const handleGradingComplete = async () => {
    ...
    setCompletedSteps(prev => new Set(prev).add(fullCurvesWorkbookSteps[currentStep].id));
    
    setTimeout(() => {
      setUploadSuccess(false);
      setSelectedFile(null);
      if (currentStep < fullCurvesWorkbookSteps.length - 1) {
  ...
  const goToNextStep = () => { if (currentStep < fullCurvesWorkbookSteps.length - 1) setCurrentStep(currentStep + 1); };

  const currentWorksheet = fullCurvesWorkbookSteps[currentStep];
  const progressPercentage = (completedSteps.size / fullCurvesWorkbookSteps.length) * 100;
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8 text-center">
          <h2 className={`text-3xl font-bold ${isKidsMode ? 'text-transparent bg-clip-text bg-gradient-to-r from-rose-600 to-orange-500' : 'text-gray-900'}`}>
            Uppercase Full Curves Letters
          </h2>
          <p className={`mt-2 text-lg ${isKidsMode ? 'text-rose-700' : 'text-gray-600'}`}>
            Let's master writing uppercase letters with full curves: C, G, O, Q, and S!
          </p>
        </div>

        {isKidsMode && (
          <div className="border-0 shadow-xl mb-8 bg-gradient-to-r from-rose-500 to-orange-500 text-white overflow-hidden rounded-2xl">
            <div className="pt-6 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  <div className="text-2xl font-bold">{completedSteps.size} / {fullCurvesWorkbookSteps.length}</div>
            ...
            <div className={`border-0 shadow-xl overflow-hidden rounded-2xl ${isKidsMode ? `bg-gradient-to-br ${currentWorksheet.color} text-white` : 'bg-white border'}`}>
              ...
              <div className="p-6 pt-0 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold">{currentStep + 1} of {fullCurvesWorkbookSteps.length}</span>
                  <Button onClick={goToNextStep} disabled={currentStep === fullCurvesWorkbookSteps.length - 1} variant="outline">Next<ChevronRight className="h-5 w-5 ml-2" /></Button>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="p-6 border-0 shadow-xl rounded-2xl bg-white/50">
              ...
              <div className="space-y-2">
                {fullCurvesWorkbookSteps.map((step, index) => (
                  <button
                    key={step.id}
