// app/practice/uppercase-curve-line/UpperspaceCurveLineClient.tsx
'use client';
...
// This array defines the practice steps for the uppercase curve and line letters workbook.
const curveLineWorkbookSteps: WorksheetStep[] = [
  {
    id: 'letter-b',
    title: 'Worksheet 2.2.1: Letter B',
    friendlyTitle: 'The letter B!',
    description: 'Practice the vertical line and two curves of the letter "B".',
    kidsDescription: "Let's draw a straight line and two round bumps to make a \"B\"!",
    level: 2,
  ...
  {
    id: 'letter-d',
    title: 'Worksheet 2.2.2: Letter D',
  ...
  {
    id: 'letter-p',
    title: 'Worksheet 2.2.3: Letter P',
    friendlyTitle: 'The letter P!',
    description: 'Practice the vertical line and single curve at the top of the letter "P".',
  ...
  {
    id: 'letter-r',
    title: 'Worksheet 2.2.4: Letter R',
  ...
  return (
    <div className="space-y-3">
      <div className={`border-2 border-dashed rounded-xl p-8 text-center transition-all duration-300 cursor-pointer ${dragActive ? isKidsMode ? 'border-purple-400 bg-gradient-to-br from-purple-100 to-pink-100 scale-105' : 'border-blue-400 bg-blue-50' : isKidsMode ? 'border-purple-300 hover:border-purple-400 hover:bg-gradient-to-br hover:from-purple-50 hover:to-pink-50 hover:scale-102' : 'border-gray-300 hover:border-gray-400 hover:bg-gray-50'} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        onClick={() => { if (!disabled && !uploading) document.getElementById('file-upload-curve-line')?.click(); }}>
        ...
        <Input id="file-upload-curve-line" type="file" accept="image/jpeg,image/png,image/jpg" onChange={handleChange} className="hidden" disabled={disabled || uploading} />
      </div>
      {error && (
...
export default function UpperspaceCurveLineClient({ user, profile }: PracticePageClientProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const currentWorksheet = curveLineWorkbookSteps[currentStep];
  ...
  const handleGradingComplete = async () => {
    ...
    setCompletedSteps(prev => new Set(prev).add(curveLineWorkbookSteps[currentStep].id));
    
    setTimeout(() => {
      setUploadSuccess(false);
      setSelectedFile(null);
      if (currentStep < curveLineWorkbookSteps.length - 1) {
  ...
  const goToNextStep = () => { if (currentStep < curveLineWorkbookSteps.length - 1) setCurrentStep(currentStep + 1); };

  const currentWorksheet = curveLineWorkbookSteps[currentStep];
  const progressPercentage = (completedSteps.size / curveLineWorkbookSteps.length) * 100;
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8 text-center">
          <h2 className={`text-3xl font-bold ${isKidsMode ? 'text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-teal-500' : 'text-gray-900'}`}>
            Uppercase Curve & Line Letters
          </h2>
          <p className={`mt-2 text-lg ${isKidsMode ? 'text-blue-700' : 'text-gray-600'}`}>
            Let's master writing uppercase letters with curves and lines: B, D, P, and R!
          </p>
        </div>
            ...
            <div className="pt-6 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  <div className="text-2xl font-bold">{completedSteps.size} / {curveLineWorkbookSteps.length}</div>
            ...
            <div className={`border-0 shadow-xl overflow-hidden rounded-2xl ${isKidsMode ? `bg-gradient-to-br ${currentWorksheet.color} text-white` : 'bg-white border'}`}>
              ...
              <div className="p-6 pt-0 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold">{currentStep + 1} of {curveLineWorkbookSteps.length}</span>
                  <Button onClick={goToNextStep} disabled={currentStep === curveLineWorkbookSteps.length - 1} variant="outline">Next<ChevronRight className="h-5 w-5 ml-2" /></Button>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="p-6 border-0 shadow-xl rounded-2xl bg-white/50">
              ...
              <div className="space-y-2">
                {curveLineWorkbookSteps.map((step, index) => (
                  <button
                    key={step.id}
