// app/practice/uppercase-diagonal-lines/UpperspaceDiagonalLinesClient.tsx
'use client';
...
// This array defines the practice steps for the uppercase diagonal lines workbook.
const diagonalLinesWorkbookSteps: WorksheetStep[] = [
  {
    id: 'letter-a',
    title: 'Worksheet 2.4.1: Letter A',
  ...
  {
    id: 'letter-k',
    title: 'Worksheet 2.4.2: Letter K',
    friendlyTitle: 'The letter K!',
    description: 'Master the vertical line and two diagonal lines of the letter "K".',
  ...
  {
    id: 'letter-m',
    title: 'Worksheet 2.4.3: Letter M',
  ...
  {
    id: 'letter-n',
    title: 'Worksheet 2.4.4: Letter N',
  ...
  {
    id: 'letter-v',
    title: 'Worksheet 2.4.5: Letter V',
    friendlyTitle: 'The letter V!',
    description: 'Practice the two diagonal lines of the letter "V".',
  ...
  {
    id: 'letter-w',
    title: 'Worksheet 2.4.6: Letter W',
    friendlyTitle: 'The letter W!',
    description: 'Master the four diagonal lines of the letter "W".',
  ...
  {
    id: 'letter-x',
    title: 'Worksheet 2.4.7: Letter X',
    friendlyTitle: 'The letter X!',
    description: 'Practice the two crossing diagonal lines of the letter "X".',
  ...
  {
    id: 'letter-y',
    title: 'Worksheet 2.4.8: Letter Y',
    friendlyTitle: 'The letter Y!',
    description: 'Master the two diagonal lines and one vertical line of the letter "Y".',
  ...
  {
    id: 'letter-z',
    title: 'Worksheet 2.4.9: Letter Z',
    friendlyTitle: 'The letter Z!',
    description: 'Practice the two horizontal lines and one diagonal line of the letter "Z".',
  ...
  return (
    <div className="space-y-3">
      <div className={`border-2 border-dashed rounded-xl p-8 text-center transition-all duration-300 cursor-pointer ${dragActive ? isKidsMode ? 'border-purple-400 bg-gradient-to-br from-purple-100 to-pink-100 scale-105' : 'border-blue-400 bg-blue-50' : isKidsMode ? 'border-purple-300 hover:border-purple-400 hover:bg-gradient-to-br hover:from-purple-50 hover:to-pink-50 hover:scale-102' : 'border-gray-300 hover:border-gray-400 hover:bg-gray-50'} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        onClick={() => { if (!disabled && !uploading) document.getElementById('file-upload-diagonal-lines')?.click(); }}>
        ...
        <Input id="file-upload-diagonal-lines" type="file" accept="image/jpeg,image/png,image/jpg" onChange={handleChange} className="hidden" disabled={disabled || uploading} />
      </div>
      {error && (
...
export default function UpperspaceDiagonalLinesClient({ user, profile }: PracticePageClientProps) {
  ...
  const simulateAIAnalysis = (file: File) => {
    ...
    return {
      ...
      feedbackTip: 'Great job on your diagonal lines! Try to keep them at consistent angles.',
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const currentWorksheet = diagonalLinesWorkbookSteps[currentStep];
  ...
  const handleGradingComplete = async () => {
    ...
    setCompletedSteps(prev => new Set(prev).add(diagonalLinesWorkbookSteps[currentStep].id));
    
    setTimeout(() => {
      setUploadSuccess(false);
      setSelectedFile(null);
      if (currentStep < diagonalLinesWorkbookSteps.length - 1) {
  ...
  const goToNextStep = () => { if (currentStep < diagonalLinesWorkbookSteps.length - 1) setCurrentStep(currentStep + 1); };

  const currentWorksheet = diagonalLinesWorkbookSteps[currentStep];
  const progressPercentage = (completedSteps.size / diagonalLinesWorkbookSteps.length) * 100;
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8 text-center">
          <h2 className={`text-3xl font-bold ${isKidsMode ? 'text-transparent bg-clip-text bg-gradient-to-r from-amber-600 to-yellow-500' : 'text-gray-900'}`}>
            Uppercase Diagonal Lines Letters
          </h2>
          <p className={`mt-2 text-lg ${isKidsMode ? 'text-amber-700' : 'text-gray-600'}`}>
            Let's master writing uppercase letters with diagonal lines: A, K, M, N, V, W, X, Y, and Z!
          </p>
        </div>

        {isKidsMode && (
          <div className="border-0 shadow-xl mb-8 bg-gradient-to-r from-amber-500 to-yellow-500 text-white overflow-hidden rounded-2xl">
            <div className="pt-6 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  <div className="text-2xl font-bold">{completedSteps.size} / {diagonalLinesWorkbookSteps.length}</div>
            ...
            <div className={`border-0 shadow-xl overflow-hidden rounded-2xl ${isKidsMode ? `bg-gradient-to-br ${currentWorksheet.color} text-white` : 'bg-white border'}`}>
              ...
              <div className="p-6 pt-0 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold">{currentStep + 1} of {diagonalLinesWorkbookSteps.length}</span>
                  <Button onClick={goToNextStep} disabled={currentStep === diagonalLinesWorkbookSteps.length - 1} variant="outline">Next<ChevronRight className="h-5 w-5 ml-2" /></Button>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="p-6 border-0 shadow-xl rounded-2xl bg-white/50">
              ...
              <div className="space-y-2">
                {diagonalLinesWorkbookSteps.map((step, index) => (
                  <button
                    key={step.id}
