// app/practice/uppercase-mixed-formation/UpperspaceMixedFormationClient.tsx
'use client';
...
// This array defines the practice steps for the uppercase mixed formation workbook.
const mixedFormationWorkbookSteps: WorksheetStep[] = [
  {
    id: 'letter-j',
    title: 'Worksheet 2.5.1: Letter J',
    friendlyTitle: 'The letter J!',
    description: 'Practice the vertical line and curve of the letter "J".',
    kidsDescription: "Let's draw a straight line with a hook at the bottom to make a \"J\"!",
    level: 2,
  ...
  {
    id: 'letter-u',
    title: 'Worksheet 2.5.2: Letter U',
    friendlyTitle: 'The letter U!',
    description: 'Master the two vertical lines and curve of the letter "U".',
  ...
  return (
    <div className="space-y-3">
      <div className={`border-2 border-dashed rounded-xl p-8 text-center transition-all duration-300 cursor-pointer ${dragActive ? isKidsMode ? 'border-purple-400 bg-gradient-to-br from-purple-100 to-pink-100 scale-105' : 'border-blue-400 bg-blue-50' : isKidsMode ? 'border-purple-300 hover:border-purple-400 hover:bg-gradient-to-br hover:from-purple-50 hover:to-pink-50 hover:scale-102' : 'border-gray-300 hover:border-gray-400 hover:bg-gray-50'} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        onClick={() => { if (!disabled && !uploading) document.getElementById('file-upload-mixed-formation')?.click(); }}>
        ...
        <Input id="file-upload-mixed-formation" type="file" accept="image/jpeg,image/png,image/jpg" onChange={handleChange} className="hidden" disabled={disabled || uploading} />
      </div>
      {error && (
...
export default function UpperspaceMixedFormationClient({ user, profile }: PracticePageClientProps) {
  ...
  const simulateAIAnalysis = (file: File) => {
    ...
    return {
      ...
      feedbackTip: 'Great job on these special letters! Focus on making the curves smooth and consistent.',
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const currentWorksheet = mixedFormationWorkbookSteps[currentStep];
  ...
  const handleGradingComplete = async () => {
    ...
    setCompletedSteps(prev => new Set(prev).add(mixedFormationWorkbookSteps[currentStep].id));
    
    setTimeout(() => {
      setUploadSuccess(false);
      setSelectedFile(null);
      if (currentStep < mixedFormationWorkbookSteps.length - 1) {
  ...
  const goToNextStep = () => { if (currentStep < mixedFormationWorkbookSteps.length - 1) setCurrentStep(currentStep + 1); };

  const currentWorksheet = mixedFormationWorkbookSteps[currentStep];
  const progressPercentage = (completedSteps.size / mixedFormationWorkbookSteps.length) * 100;
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8 text-center">
          <h2 className={`text-3xl font-bold ${isKidsMode ? 'text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 to-teal-500' : 'text-gray-900'}`}>
            Uppercase Mixed Formation Letters
          </h2>
          <p className={`mt-2 text-lg ${isKidsMode ? 'text-cyan-700' : 'text-gray-600'}`}>
            Let's master writing special uppercase letters: J and U!
          </p>
        </div>

        {isKidsMode && (
          <div className="border-0 shadow-xl mb-8 bg-gradient-to-r from-cyan-500 to-teal-500 text-white overflow-hidden rounded-2xl">
            <div className="pt-6 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  <div className="text-2xl font-bold">{completedSteps.size} / {mixedFormationWorkbookSteps.length}</div>
            ...
            <div className={`border-0 shadow-xl overflow-hidden rounded-2xl ${isKidsMode ? `bg-gradient-to-br ${currentWorksheet.color} text-white` : 'bg-white border'}`}>
              ...
              <div className="p-6 pt-0 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold">{currentStep + 1} of {mixedFormationWorkbookSteps.length}</span>
                  <Button onClick={goToNextStep} disabled={currentStep === mixedFormationWorkbookSteps.length - 1} variant="outline">Next<ChevronRight className="h-5 w-5 ml-2" /></Button>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="p-6 border-0 shadow-xl rounded-2xl bg-white/50">
              ...
              <div className="space-y-2">
                {mixedFormationWorkbookSteps.map((step, index) => (
                  <button
                    key={step.id}
